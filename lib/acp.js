...
// // 签名
// ACP.sign = function(data) {
// 	const sign = crypto.createSign('RSA-SHA256')
// 	sign.update(data)
// 	return sign.sign(ACP.MER_KEY, 'hex')
// }

// // 验签
// ACP.verify = function(data, signature) {
// 	const verify = crypto.createVerify('RSA-SHA256')
// 	verify.update(data)
// 	return verify.verify(ACP.MER_CER, signature, 'hex')
// }
...
ACP.customEncrypt = function(data, keys) {
	...
	if (encryptedInfoSb.length > 0) {
		console.log('组装的customerInfo encryptedInfo明文:', encryptedInfoSb.join('&'));
		sf.push('encryptedInfo=' + ACP.encryptData(encryptedInfoSb.join('&')))
	}
	console.log('组装的customerInfo明文:', '{' + sf.join('&') + '}');
